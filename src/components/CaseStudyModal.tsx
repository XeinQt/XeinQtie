...
export const CaseStudyModal: React.FC<CaseStudyModalProps> = ({ isOpen, onClose }) => {
  ...
  // Featured Project: KABAN
  const kaban = projectsData.find(p => p.id === 'kaban') || projectsData[0];
  const screenshots = kaban.screenshots || [];
  const currentImage = screenshots.length > 0 ? screenshots[selectedScreenshotIdx]?.url : kaban.image;
  const currentCaption = screenshots.length > 0 ? screenshots[selectedScreenshotIdx]?.caption : kaban.description;
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono font-semibold tracking-widest text-zinc-400 dark:text-zinc-500 uppercase">
                FEATURED CASE STUDY · {kaban.year}
              </span>
        ...
        <div className="p-6 sm:p-8 overflow-y-auto space-y-6 text-xs sm:text-sm text-zinc-600 dark:text-zinc-300 font-sans">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6 animate-fade-in">
              ...
              {/* Metrics Row */}
              {kaban.caseStudy.metrics && (
                <div className="grid grid-cols-3 gap-3">
                  {kaban.caseStudy.metrics.map((m, idx) => (
              ...
              {kaban.caseStudy.securityHighlights && (
                <div className="p-4 rounded-2xl bg-[#0284c7]/5 dark:bg-[#38bdf8]/5 border border-[#38bdf8]/20 space-y-2">
                  ...
                  <ul className="space-y-1 text-xs text-zinc-600 dark:text-zinc-300">
                    {kaban.caseStudy.securityHighlights.map((item, idx) => (
              ...
              {kaban.caseStudy.architecture && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {kaban.caseStudy.architecture.map((arch, i) => (
          ...
          {activeTab === 'techStack' && (
            <div className="space-y-4 animate-fade-in">
              <div className="border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800 font-sans">
                    {kaban.techStackTable?.map((row, rIdx) => (
              ...
              {/* Technologies Badges */}
              <div className="pt-2">
                <h5 className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-2">
                  Featured Stack Badges:
                </h5>
                <div className="flex flex-wrap gap-1.5">
                  {kaban.tags.map((t, i) => (
                    <TechBadge key={i} name={t} size="sm" />
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-850 border border-zinc-200/70 dark:border-zinc-800 text-[11px] text-zinc-500 dark:text-zinc-400 flex items-center justify-between">
            <div>
              <strong>Authors & Engineering Credits:</strong> Developed by Archie Boiser and Rico Alentijo.
            </div>
            <div className="text-[10px] font-mono text-zinc-400">
              Institutional Release 2026
            </div>
          </div>
